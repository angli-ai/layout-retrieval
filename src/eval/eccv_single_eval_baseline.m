...
resultname = {'det20_soft', '../eval-data/output-sunrgbd-20-det'};
...
output = table(queries', gt_ranks', det_ranks', det_ranks_soft', resultrank{1}');
...
det_anno_table = table(rowname', gt_res', det_res', proposed_res(1, :)');
print_table_tex(det_anno_table);
tableres = eccv_plot_curves(...
    {gt_ranks, det_ranks, resultrank{1}}, ...
    ntest, ...
    {'gt', 'det', 'ours det soft max'}, ...
    {'--', '--', '-'});
saveas(h, 'result.png');