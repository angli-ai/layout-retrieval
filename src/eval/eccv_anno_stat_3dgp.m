...
outputs = {};
load('output3dgp', 'output3dgp');
outputs{1} = output3dgp;
legendnames = {'3dgp'};
...
for oo = 1:1
    ...
    relationpath = '../data/relations-3dgp-all';
    textpath = '../text2relations/written_text-3dgpv2.txt';
    names = output.query;
    ...
    cnt = cnt(no_against);
    no_on = ~strcmp('on', unique_rels);
    unique_rels = unique_rels(no_on);
    cnt = cnt(no_on);
    stats{oo} = [];
...
unique_rels = [stats{1}.rels];
unique_rels = unique(unique_rels);
unique_cnts = zeros(length(unique_rels), 1);
for oo = 1:1
    rels = stats{oo}.rels;
    cnts = stats{oo}.cnt;
    for i = 1:length(rels)
...
h = figure(3);
[sorted_cnt, sorted_index] = sort(unique_cnts, 'descend');
ntest = size(outputs{1}, 1);
sorted_cnt = unique_cnts(sorted_index) / 40;
c = lines;
...
set(gca,'TickLabelInterpreter','none')
xlabel('Spatial Relation');
ylabel('Frequency (# per Query)');
set(gca, 'fontsize', 20);
% legend(legendnames);
saveas(h, 'eccv-rel-stat-3dgp.eps', 'ps2c');

% plot obj cnt
unique_objs = [stats{1}.objs];
unique_objs = unique(unique_objs);
unique_objcnts = zeros(length(unique_objs), 1);
for oo = 1:1
    objs = stats{oo}.objs;
...
h = figure(4);
[sorted_cnt, sorted_index] = sort(unique_objcnts, 'descend');
ntest = size(outputs{1}, 1);
sorted_cnt = unique_objcnts(sorted_index) / 40;
c = lines;
hb = bar(sorted_cnt, 0.5, 'stacked');
hb(1).FaceColor = c(6, :);
ax = gca;
...
ylim([0 1]);
% legend(legendnames);
saveas(h, 'eccv-obj-stat-3dgp.eps', 'ps2c');